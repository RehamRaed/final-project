      ...
      <header className="mb-8 border-b border-border pb-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <h1 className="text-3xl md:text-4xl font-extrabold text-text-primary mb-3">{courseData.title}</h1>

            {courseData.description && (
              <p className="text-lg text-text-secondary mb-4">{courseData.description}</p>
            )}
            ...
            <div className="flex flex-wrap gap-4 text-sm text-text-secondary">
              ...
              {courseData.level && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-3 py-1 bg-blue-100 text-primary rounded-full font-semibold">{courseData.level}</span>
                </div>
              )}
        ...
        <div className="mt-6">
          ...
          <div className="w-full bg-border rounded-full h-4 overflow-hidden">
            <div className="bg-primary h-full rounded-full transition-all duration-500" style={{ width: `${lessonProgressPercent}%` }} />
          </div>

          <p className="text-sm text-text-secondary mt-2">{completedLessons} of {lessons.length} lessons completed</p>
        </div>
      </header>

      {lessons.length === 0 ? (
        <p className="text-center text-text-secondary py-16">No lessons available for this course yet.</p>
      ) : (
        <div className="flex justify-center mt-10">
          <button onClick={handleStartLearning} className="px-8 py-3 bg-primary cursor-pointer text-white text-lg font-bold rounded-xl shadow-lg transition-all hover:scale-105 active:scale-95">
